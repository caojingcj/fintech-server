...
public class CreditVettingServiceImpl implements CreditVettingService {
    ...
    public CreditVettingResultEnum creditVetting(String orderId) {
        ...
        // 用户信息监测（user_info_check）/用户黑名单信息（check_black_info）/phone_gray_score
        if (logMoxieinfo.getMoxieStatus() == 5 && jv.validate(logMoxieinfo.getMoxieContent())) {
            Object object = jsonTools.getObjectByJson(logMoxieinfo.getMoxieContent(),
        ...
        // 拒绝 - 魔蝎运营商报告 1.5风险分析摘要-魔蝎变量 申请人姓名+身份证号码是否出现在法院黑名单
        // 基本信息校验点（basic_check_items）/is_name_and_idcard_in_court_black
        if (logMoxieinfo.getMoxieStatus() == 5 && jv.validate(logMoxieinfo.getMoxieContent())) {
            Object object = jsonTools.getObjectByJson(logMoxieinfo.getMoxieContent(), "basic_check_items",
            ...
            if (object != null && !objToArrayList(object, "is_name_and_idcard_in_court_black")) {
                logOrder(orderId, CreditVettingResultEnum.拒绝.getValue(),
                        "拒绝 - 魔蝎运营商报告  1.5风险分析摘要-魔蝎变量  申请人姓名+身份证号码出现在法院黑名单   基本信息校验点");
                return CreditVettingResultEnum.拒绝;
            }
        }
        // 拒绝 - 魔蝎运营商报告 1.5风险分析摘要-魔蝎变量 申请人姓名+身份证号码是否出现在金融机构黑名单
        // 基本信息校验点（basic_check_items）/is_name_and_idcard_in_finance_black
        if (logMoxieinfo.getMoxieStatus() == 5 && jv.validate(logMoxieinfo.getMoxieContent())) {
            Object object = jsonTools.getObjectByJson(logMoxieinfo.getMoxieContent(), "basic_check_items",
                    ConstantInterface.Enum.TypeEnum.arrayList);
            if (object != null && !objToArrayList(object, "is_name_and_idcard_in_finance_black")) {
                logOrder(orderId, CreditVettingResultEnum.拒绝.getValue(),
                        "拒绝 - 魔蝎运营商报告  1.5风险分析摘要-魔蝎变量  申请人姓名+手机号码是现在金融机构黑名单  基本信息校验点");
                return CreditVettingResultEnum.拒绝;
            }
        }
        // 拒绝 - 魔蝎运营商报告 1.5风险分析摘要-魔蝎变量 申请人姓名+手机号码是否出现在金融机构黑名单
        // 基本信息校验点（basic_check_items）/is_name_and_mobile_in_finance_black
        if (logMoxieinfo.getMoxieStatus() == 5 && jv.validate(logMoxieinfo.getMoxieContent())) {
            Object object = jsonTools.getObjectByJson(logMoxieinfo.getMoxieContent(), "basic_check_items",
                    ConstantInterface.Enum.TypeEnum.arrayList);
        ...
        if (logMoxieinfo.getMoxieStatus() == 5 && jv.validate(logMoxieinfo.getMoxieContent())) {
            Object object = jsonTools.getObjectByJson(logMoxieinfo.getMoxieContent(), "call_risk_analysis",
                    ConstantInterface.Enum.TypeEnum.arrayList);
            if (object != null) {
        ...
        // 拒绝 - 魔蝎运营商报告 1.6活跃分析摘要 通话活跃天数 近3个月-通话活跃天数 <=30天
        // 4.1分析点枚举（近1/3/6月）(active_degree)/call_day
        if (logMoxieinfo.getMoxieStatus() == 5 && jv.validate(logMoxieinfo.getMoxieContent())) {
            Object object1 = jsonTools.getObjectByJson(logMoxieinfo.getMoxieContent(), "active_degree",
                    ConstantInterface.Enum.TypeEnum.arrayList);
            if (object1 != null) {
        ...
        if (logMoxieinfo.getMoxieStatus() == 5 && jv.validate(logMoxieinfo.getMoxieContent())) {
            Object object = jsonTools.getObjectByJson(logMoxieinfo.getMoxieContent(), "call_contact_detail",
                    ConstantInterface.Enum.TypeEnum.arrayList);
            if (object != null) {
                boolean flag = false;
                ArrayList<Map<String, Object>> list = (ArrayList<Map<String, Object>>) object;
        ...
        if (jv.validate(logMozhanginfo.getMozhangContent())) {
            Object object = jsonTools.getObjectByJson(logMozhanginfo.getMozhangContent(), "black_info_detail",
                    ConstantInterface.Enum.TypeEnum.map);
